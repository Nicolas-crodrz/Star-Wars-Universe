...
class Particula {
  ...
  update() {
    ...
    if (distance < mouse.radius + this.size) {
      // Si la distancia es menor a la suma de los radios

      if (mouse.x < this.x && this.x < canvas.width - this.size * 10) {
        // Si el mouse esta a la izquierda de la particula

        this.x += 10; // Mover la particula a la derecha
      }
      if (mouse.x > this.x && this.x > this.size * 10) {
        // Si el mouse esta a la derecha de la particula

        this.x -= 10; // Mover la particula a la izquierda
      }
      if (mouse.y < this.y && this.y < canvas.height - this.size * 10) {
        // Si el mouse esta arriba de la particula

        this.y += 10; // Mover la particula abajo
      }
      if (mouse.y > this.y && this.y > this.size * 10) {
        // Si el mouse esta abajo de la particula
...
function init() {
  ...
  for (let i = 0; i < numeroParticulas; i++) {
    ...
    ArrayParticulas.push(
      new Particula(x, y, direccionX, direccionY, size, color)
...
window.addEventListener("recolocar", function () {
  // Evento recolocar
